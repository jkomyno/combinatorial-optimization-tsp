...
namespace cli {
    struct CLIArgs : public argparse::Args {
        ...
        double& crossover_rate =
            kwarg("c,crossover-rate", "GA crossover rate").set_default(0.9254767404002548);

        unsigned int& mu = kwarg("mu", "GA population pool size").set_default(40u);

        unsigned int& lambda = kwarg("lambda", "GA offspring pool size").set_default(60u);

        unsigned int& max_gen_no_improvement =
            kwarg("N,max-gen-no-improvement",
                  "Maximum number of generations without solution improvement")
                .set_default(177u);

        unsigned int& max_gen =
            kwarg("M,max-gen", "Maximum number of generations").set_default(408u);

        unsigned int& k = kwarg("k", "Size of the GA tournament").set_default(13u);
