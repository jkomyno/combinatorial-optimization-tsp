...
inline void CPLEXModel<T>::setup_lp() noexcept {
	...
	for (int k = 1; k < this->N; ++k) {
		...
		int matbeg = 0;
		CHECKED_CPX_CALL(CPXaddrows, env, lp, 0, 1, sum1 + sum2, &right_side, &sense,
			&matbeg, left_side.data(), coeff.data(), nullptr, nullptr);
	}
	...
	for (int i = 0; i < this->N; ++i) {
		for (int j = 1; j < this->N; ++j) {
			if (i != j) {
				int left_side[2] {
					x_variable_mat.at(i, j),
					y_variable_mat.at(i, j)
				};
				double coeff[2] {
					1,
					-this->N + 1
...
inline void CPLEXModel<T>::add_column(char variable, char type, double lower_bound,
	...
	// Add the new column
	CHECKED_CPX_CALL(CPXnewcols, env, lp,
		             n_variables_to_add, &objective_coefficient, &lower_bound, &upper_bound,
		             &type, &raw_variable_name);
}
...
inline std::optional<T> CPLEXModel<T>::get_solution() const {
	try {
		...
		// CPXENVptr env: The pointer to the CPLEX environment as returned by one of the CPXopenCPLEX routines.
		// CPXLPptr lp: A pointer to a CPLEX LP problem object as returned by CPXcreateprob()
		// double *x: An array to receive the values of the primal variables for the problem.
		//            This array must be of length at least (end - begin + 1).
		//            If successful, x[0] through x[end-begin] will contain the solution values.
		// int begin: An integer indicating the beginning of the range of variable values to be returned.
		// int end: An integer indicating the end of the range of variable values to be returned.
